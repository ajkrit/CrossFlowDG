...
from model import build_model, build_model_eval
...
from utils.dgutil import train_valid_target_eval_names, DG_accuracy, DG_target_accuracy, img_param_init
...
def parse_option():
    parser = argparse.ArgumentParser('DGFM training and evaluation script', add_help=False)
    ...
    parser.add_argument('--eval', type=str, help='Perform evaluation only on ckpt found in given path')
...
def main(config):
    ...
    if config.MODEL.RESUME:
        ...
        train_acc, valid_acc, target_acc = validate(config, logger, data_loader_val, model, epoch)
        logger.info(f"Accuracy of the network on the TARGET test images: {target_acc:.1f}%")
        if model_ema is not None:
            train_acc_ema, valid_acc_ema, target_acc_ema = validate(config, logger, data_loader_val, model_ema.ema)
    ...
    if config.EVAL is not None:
        logger.info(f"Loading checkpoint for evaluation from: {config.EVAL}")
        
        checkpoint = torch.load(config.EVAL, map_location='cpu')
        ...
        # Run evaluation
        target_acc = evaluate(config, logger, data_loader_val, model)
        logger.info(f"Accuracy of the network on the TARGET test images: {target_acc:.1f}%")
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None):
    ...
    for idx in range(config.steps_per_epoch):
        ...
        input_ids = torch.cat([data[1]['input_ids'].long() for data in minibatches])
        attention_mask = torch.cat([data[1]['attention_mask'].long() for data in minibatches])
        targets = torch.cat([data[2].long() for data in minibatches])
        ...
        with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
            _, losses, _, _ = model(samples, input_ids, attention_mask, y, num_steps=config.FLOW.NUM_STEPS, epoch=epoch)
        ...
        loss =  (
                ...
                0.4*losses['flow_matching'] +
                10*losses['classifier']
        )
...
@torch.no_grad()
def validate(config, logger, data_loader, model, epoch=None):
    ...
    for item in acc_type_list:
        acc_record[item] = np.mean(np.array(
                    [DG_accuracy(config, logger, model, data_loader[i], item, epoch) for i in eval_name_dict[item]]
                ))
...
@torch.no_grad()
def evaluate(config, logger, data_loader, model):
    ...
    for item in acc_type_list:
        acc_record[item] = np.mean(np.array(
                    [DG_target_accuracy(config, logger, model, data_loader[i]) for i in eval_name_dict[item]]
                ))
    logger.info(f" * target {acc_record['target']:.4f}")
